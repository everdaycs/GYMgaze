...
**重要更新**: 训练流程已模块化，分为独立的`collect_training_data.py`（数据收集）和`train_model.py`（模型训练）脚本。
...
**实现：** `collect_training_data.py` + `train_model.py`
...
# 安装其他依赖
pip install tqdm matplotlib scikit-learn opencv-python
```
...
python collect_training_data.py --episodes 100

# 收集更多数据以提高性能
python collect_training_data.py --episodes 500 --max-steps 500

# 自定义数据目录
python collect_training_data.py --episodes 200 --data-dir ./my_training_data
```

**数据结构：**
```
每个样本包含：
- occupancy: 占用栅格地图 (400x400, uint8)
- ground_truth: 真实障碍物地图 (800x800, uint8) 
- robot_pos: 机器人位置 (2,)
- visit_count: 访问计数 (400x400, uint16)
- step: 时间步
```
...
# 使用自定义数据目录
python train_model.py --data-dir ./my_training_data --epochs 100
...
```bash
# 评估训练好的模型（TODO: 实现评估脚本）
python evaluate_model.py --model-path ./obstacle_diffusion_model.pth
```
...
**之前的单体脚本** `train_diffusion_model.py` 已拆分为：

1. **`collect_training_data.py`** - 专用数据收集
   - 独立的数据收集流程
   - 修复了RingSonarCore属性访问问题
   - 支持自定义episode数量和步数

2. **`train_model.py`** - 专用模型训练
   - 独立的模型训练流程
   - 修复了数据集维度不匹配问题
   - 自动处理ground_truth下采样
...
**🐛 数据收集脚本修复：**
- 修复了`AttributeError: 'RingSonarCore' object has no attribute 'world_grid'`
- 使用正确的`world_width`、`world_height`和像素尺寸`width`、`height`
- 正确处理障碍物数据格式`('rect', (x, y, w, h))`

**🐛 训练脚本修复：**
- 修复了维度不匹配：`occupancy`(400x400) vs `ground_truth`(800x800)
- 添加自动下采样，使用`cv2.resize`将ground_truth调整到栅格分辨率
- 添加了缺失的`cv2`导入
...
```bash
# 数据收集（修复版）
python collect_training_data.py --episodes 100 --max-steps 300

# 模型训练（修复版）
python train_model.py --epochs 50 --batch-size 16

# 完整流程
python collect_training_data.py --episodes 200
python train_model.py --data-dir ./diffusion_training_data --epochs 100
```
